...
#CHAPTER 1: TONIC
```{r}
setwd("/Volumes/Group Kok/Ariane /Ariane_Triple-B/Data/Clinical/Projects/BMI project/Final data files")
tonicdf <- read.csv ("TONIC12_clinicaldata_BMIproject_20260122.csv")
View(tonicdf)
```
...
tidy biopsy site categories
```{r}
glimpse(tonicdf$baseline_biopsy_site)

tonicdf <- tonicdf %>%
  mutate (baseline_biopsy_site_tidy = case_when (
   baseline_biopsy_site %in% c("Breast", "Mamma R", "Mamma", "Mamma L") ~ "Breast",
   baseline_biopsy_site %in% c("Skin", "Skin / thoracall wall") ~ "Skin",
   baseline_biopsy_site %in% c("Liver", "liver") ~ "Liver",
   baseline_biopsy_site %in% c("Lymphnode", "Lymph node parasternaal R",
                               "LN supraclavicular L", "LN neck R", "LN axilla rechts",
                               "Lymph node axilla L", "Lymph node, axilla L", 
                               "Lymph node ") ~ "Lymphnode",
   baseline_biopsy_site %in% c("Soft tissue", "Muscle", "Subcutaneous tissue", 
                               "Parasternale massa", 
                               "Neck  (no LN)", "soft tissue surrounding costa",
                               "Mass supraclavicular", "Glandula submandibularis",
                               "Chest wall", "Thoracal wall", "Thoraxwall") ~ "Soft tissue",
   baseline_biopsy_site %in% c("Lung", "Lung, r", "Pleural") ~ "Lung",
   baseline_biopsy_site %in% c("Peritoneum", "Stomach") ~ "Other visceral",
   TRUE ~ as.character(NA)))

tonicdf <- tonicdf %>%
  relocate(baseline_biopsy_site_tidy, .after = baseline_biopsy_site)
```
...
biopsy location and TILs
```{r}
tonicdf_filtered %>%
  group_by (baseline_biopsy_site_tidy) %>%
  filter (sTIL != "NA") %>%
  summarize (number = n(),
             median_TIL_score = median (sTIL),
             minTIL_score = min(sTIL),
             maxTIL_score = max(sTIL))

#distribution of TILs per biopsy site is sig different
kruskal.test(sTIL ~ baseline_biopsy_site_tidy, data = tonicdf_filtered)

#post-hoc testing with correction renders only LN vs Liver sig
tonicdf_filtered %>%
  dunn_test(sTIL ~ baseline_biopsy_site_tidy, p.adjust.method = "bonferroni")
```
...
interaction between TILs and BMI and influence on response labels
```{r}
tonicdf_filtered <- tonicdf_filtered %>%
  mutate (response_labels_bin = case_when(
    response_labels == "CR + PR" ~ 1,
    response_labels == "PD" ~ 0,
    TRUE ~ as.numeric(response_labels)))

model <- glm(response_labels_bin ~ TILs_median * BMI, data = tonicdf_filtered, 
             family = binomial)
...
#PFS
#BMI grouped
coxph(Surv(pfsmonths, pfs_osstat_ok) ~ BMI_grouped, data = tonicdf_filtered)


#BMI continuous
coxph(Surv(pfsmonths, pfs_osstat_ok) ~ BMI, data = tonicdf_filtered)

#OS
#BMI grouped
coxph(Surv(osmonths, osstat) ~ BMI_grouped, data = tonicdf_filtered)

#BMI continuous
coxph(Surv(osmonths, osstat) ~ BMI, data = tonicdf_filtered)
```
...
#pfs
## interaction model
cox_model <- coxph(Surv(pfs_days, pfs_osstat_ok) ~ BMI * TILs_median, 
...
#os
## interaction model
cox_model1 <- coxph(Surv(os_days, osstat) ~ BMI * TILs_median,
                    data = tonicdf_filtered)
tbl_regression(cox_model1, exponentiate = TRUE)
...
PFS
```{r}
survfit2(Surv(pfsmonths, pfs_osstat_ok) ~ BMI_grouped, data = tonicdf_filtered) %>%
  ggsurvfit () +
  labs ( x = "months",
         y = "progression-free survival probability",
         title = "kaplan-meier for pfs per BMI group") +
  add_risktable() +
  add_pvalue() + 
  theme_minimal () +
  coord_cartesian(xlim = c(0, 30)) +        # show only 0–30 months
...
PFS
```{r}
survfit2(Surv(pfsmonths, pfs_osstat_ok) ~ BMI_TIL_groups, data = tonicdf_filtered) %>%
  ggsurvfit () +
  labs ( x = "months",
         y = "progression-free survival probability",
         title = "kaplan-meier for pfs per BMI-TIL group") +
  add_risktable() +
  add_pvalue() + 
  theme_minimal () +
  coord_cartesian(xlim = c(0, 30)) +        # show only 0–30 months
...
#CHAPTER 2: TRIPLE-B
```{r}
setwd("/Volumes/Group Kok/Ariane /Ariane_Triple-B/Data/Clinical/Data files/Final data files")
tripledf <- read.csv ("Triple-B_Clinical_data_20260122_final.csv")
```
...
tidy baseline biopsy location
```{r}
tripledf <- tripledf %>%
  mutate (baseline_biopsy_site_tidy = case_when(
    baseline_biopsy_site == "Mamma" ~ "Breast",
    baseline_biopsy_site %in% c("Lung", "Pleura") ~ "Lung",
    baseline_biopsy_site == "Liver" ~ "Liver",
    baseline_biopsy_site == "Lymphnode" ~ "Lymphnode",
    baseline_biopsy_site %in% c("Soft tissue", "Chest wall", "mediastinum",
                                "Abdominal wall") ~ "Soft tissue",
    baseline_biopsy_site == "Bone" ~ "Bone",
    baseline_biopsy_site == "Skin" ~ "Skin", 
    TRUE ~ as.character (NA)))
```
...
chisq.test(tripledf_filtered_atezo$BMI_grouped, tripledf_filtered_atezo$response_labels)
```
...
interaction between TILs and BMI on response labels
```{r}
tripledf_filtered <- tripledf_filtered %>%
  mutate (response_labels_bin = case_when(
    response_labels == "CR + PR" ~ 1,
...
model <- glm(response_labels_bin ~ TILs_median * bmi.baseline, data = tripledf_filtered,
             family = binomial)
tbl_regression (model)
```
...
#distribution of TILs per biopsy site does not differ sig
kruskal.test(sTIL ~ baseline_biopsy_site_tidy, data = tripledf_filtered)
```
...
```{r}
#pfs 
## interaction coefficient for all 1st line atezo arms together
cox_model2 <- coxph(Surv(pfs_ok, pfsstat_osstat_ok) ~ TILs_median * bmi.baseline,
      data = (tripledf_filtered) %>% 
      filter (arm == "Carboplatin, Cyclophosphamide, Atezolizumab"| arm == 
             "Paclitaxel, Atezolizumab"))
tbl_regression(cox_model2)

## interaction coefficient per atezo arm
cox_model4 <- coxph(Surv(pfs_ok, pfsstat_osstat_ok) ~ TILs_median * bmi.baseline,
      data = (tripledf_filtered) %>% 
      filter (arm == "Paclitaxel, Atezolizumab"))
tbl_regression(cox_model4)
...
#os
## interaction model for all 1st line atezo arms together
cox_model3 <- coxph(Surv(os, osstat) ~ TILs_median * bmi.baseline,
      data = (tripledf_filtered) %>%  
      filter (arm == "Carboplatin, Cyclophosphamide, Atezolizumab"| arm == 
             "Paclitaxel, Atezolizumab"))
tbl_regression(cox_model3)

## interaction model for all 1st line atezo arms together
cox_model5 <- coxph(Surv(os, osstat) ~ TILs_median * bmi.baseline,
      data = (tripledf_filtered) %>%  
      filter (arm == "Paclitaxel, Atezolizumab"))
tbl_regression(cox_model5)
...
coxph(Surv(os, osstat) ~ arm + age.rand + Answer.CPS + bmi.baseline * TILs_median 
              + bl.ldh.dic + who +
...
so they are included in multivariable analysis
```{r}
coxph(Surv(pfs_ok, pfsstat_osstat_ok) ~ arm + age.rand + Answer.CPS + bmi.baseline * TILs_median +
...
does BMI interact with TILs and influence CB?
```{r}
lm_model <- glm (CB_bin ~ TILs30 * BMI_at_diagnosis, data = beldf_filtered %>% 
            filter (Cohort != "3B"))
tbl_regression(lm_model, exponentiate = TRUE)
```
...
model <- glm(pCR_bin ~ BMI_at_diagnosis * TIL_mean, data = 
              beldf_filtered %>% filter (Cohort == "3B"), family = binomial)
...
#CHAPTER 4: TONIC/TRIPLE-B COMBINED
I want to make a new df with bmi and TILs from TONIC and Triple-B combined
first make sure the variables are numeric
```{r}
tonicdf_filtered <- tonicdf_filtered %>%
  mutate (patnr = as.character (patnr),
          BMI = as.numeric (BMI),
          sTIL = as.numeric (sTIL))

tripledf_filtered <- tripledf_filtered %>%
  mutate (patnr = as.character (patnr),
          bmi.baseline = as.numeric (bmi.baseline),
          sTIL = as.numeric (sTIL))
```
...
```{r}
combinedf <- bind_rows(
  tonicdf_filtered %>% select (patnr, bmi = BMI, sTIL),
  tripledf_filtered %>% select (patnr, bmi = bmi.baseline, sTIL))
View(combinedf)
```

Now analyse relation between TILs and BMI
```{r}
cor.test(combinedf$bmi, combinedf$sTIL, method = "spearman")

ggplot(data = combinedf %>% filter (patnr != "283", sTIL != "NA"), mapping = aes (x = bmi, y = sTIL)) +
  geom_point(alpha = 0.5) +
  geom_smooth (se = FALSE) +
  labs(
    x = "BMI at baseline",
    y = "TIL-score",
    title = "TONIC & Triple-B combined"
  ) 
```

Make bmi groups to analyse odds ratio and medians per group
```{r}
combinedf <- combinedf %>%
  mutate (BMI_grouped = case_when(
                        bmi >= 18.5 & bmi < 25 ~ "low BMI",
                        bmi >= 25 ~ "high BMI",
                        TRUE ~ as.character(NA)))
glimpse(combinedf$BMI_grouped)
...
combinedf <- combinedf %>%
  mutate (TILs_median = case_when(
    ...
    sTIL >= 5 ~ ">=5%",
    TRUE ~ as.character(NA)))
glimpse(combinedf$TILs_median)
```

```{r}
combinedf %>%
  group_by (BMI_grouped) %>%
  filter (sTIL != "NA") %>%
  summarize (medianTIL = median (sTIL),
             minTIL = min (sTIL),
             maxTIL = max (sTIL),
             count = n ())
     
wilcox.test (sTIL ~ BMI_grouped, data = combinedf, correct = FALSE)

ggplot (data = combinedf, mapping = aes (x = sTIL)) +
  geom_histogram (binwidth = 5, color = "white", 
                  fill = "lightblue") +
  stat_bin(aes(y = ..count..), geom = "line", binwidth = 5, color = "red", size = 1) +
  facet_wrap(~BMI_grouped) +
  labs(
    x = "TIL-score",
    y = "n",
    title = "distribution of TILs per BMI group"
  )
```
